...
function startProfileForSpan(span) {
  ...
  if (debugBuild.DEBUG_BUILD) {
    core.logger.log(`[Profiling] started profiling span: ${core.spanToJSON(span).description}`);
  }
  ...
  async function onProfileHandler() {
    ...
    return profiler
      ...
      .then((profile) => {
        ...
        if (debugBuild.DEBUG_BUILD) {
          core.logger.log(`[Profiling] stopped profiling of span: ${core.spanToJSON(span).description}`);
        }
      ...
      .catch(error => {
        if (debugBuild.DEBUG_BUILD) {
          core.logger.log('[Profiling] error while stopping profiler:', error);
        }
      });
  }
